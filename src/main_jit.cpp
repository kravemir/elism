...
static void register_printf(CodegenContext &ctx) {
    ...
    auto cft = new ::FunctionType(printf_type,IntType::get32(ctx),{"fn_default"},{});
    cft->native = true;
...
static void register_regions(CodegenContext &ctx) {
    ...
    ctx.addValue("NewRegion", new CodegenValue(new ::FunctionType(newregion_type,rtype,{"fn_default"},{}), newregion));
}
